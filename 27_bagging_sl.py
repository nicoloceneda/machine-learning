...
# Initialize a bagging object with a decision tree as base estimator
...
for i, clf, label in zip([0,1], [tree, bag], ['Tree', 'Bagging']):
...
y_bag_train_predict = bag.predict(X_train)
y_bag_test_predict = bag.predict(X_test)
...
print(f'Bag train accuracy: {accuracy_score(y_train, y_bag_train_predict):.3f}')
print(f'Bag test accuracy: {accuracy_score(y_test, y_bag_test_predict):.3f}')
