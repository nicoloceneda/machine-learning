...
# Design a custom layer

class NoisyLinear(nn.Module):

    def __init__(self, input_size, output_size, noise_stddev):
    ...
    def forward(self, x, training=False):
        ...
        if training:
            ...
            noise = torch.normal(0, self.noise_stddev, x.shape, device=device)
...
class Model(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size, noise_stddev):

        super().__init__()

        self.layer1 = NoisyLinear(input_size, hidden_size, noise_stddev)
        self.activ1 = nn.ReLU()
...
model = Model(input_size, hidden_size, output_size, noise_stddev)
...
plot_decision_regions(X=X_valid.detach().cpu().numpy(), y=y_valid.detach().cpu().numpy().astype(int), clf=model, ax=ax[2])
ax[2].set_xlabel(r'$x_1$', size=15)
ax[2].xaxis.set_label_coords(1, -0.025)
ax[2].set_ylabel(r'$x_2$', size=15)
ax[2].yaxis.set_label_coords(-0.025, 1)
fig.tight_layout()
